...
<div class="form-group">
    ...
    <textarea name="{{$jsonName}}[{{$value_key}}][{{$inputName}}]"  class="form-control ">{{$value}}</textarea>
        @error($jsonName)
...
<div class="form-group">
    <div class="col-lg-12">
        {{-- {!! Form::textarea($name, $value, ['class'=>'form-control ckeditor' ,'id' => 'ckeditor-__number__'], $attributes) !!} --}}
        <textarea __name__="{{$jsonName}}[__number__][{{$inputName}}]"  class="form-control "></textarea>
        @error($jsonName)
            <label id="{{$jsonName}}-error" class="validation-error-label" for="{{$jsonName}}">{{ $message }}</label>
        @enderror
    </div>
